(function () {
     ...
     */
        ...
        function zk($W, $D) {
            ...
            // TEMP permet de stocker des données temporaire (voir AJAX)
            ...
            function setProto(name, objF) {
                $GET('PROTO').on($GET('PROTO').functions(name, objF)[name])
            }
            function freeze() {
            ...
            function round(el, nb){
                return Math.floor(el) + (Math.round((el - Math.floor(el)) * Math.pow(10, nb)) / Math.pow(10, nb));
            }
            ...
            function PROTO() {
                var This = this,
                    OBJS = [String, Array, NODE, NodeList, HTMLElement, HTMLCollection],
                    functions = {},
                    objects = OBJS;
                /**
                ...
                This.off = function (objs, fName) {
                    return This.on(objs, fName, ZKID)
                };
                /**
        ...
        // ---------------------------------------------------------------
            ...
            function GET() {
                ...
                var middle = {
                    ...
                    both = {
                        ...
                        node: function (el, reg, type) {
                            ...
                            return $GET('NODE').create(f.get().concat(l.get()))
                        },
                        // Renvoie le service NODE
                ...
                // getBefore('div.rouge'); par exemple
                // getBefore(document.getElementById('...')); par exemple
                // getBefore(document.querySelector('...')); par exemple
                    before = {
                ...
                var children = {
                    node: function (el, nb, isAll, zkId) {
                        var nodes = ((zkId === ZKID) ? el : el.get()), res = [], r;
                        each(nodes, function () {
                            if (nb === undefined) {
                                r = toArray(this.v[(isAll === true) ? 'childNodes' : 'children'])
                            }
                            else {
                                r = This[is(nb, 'array') ? 'at' : 'first'](toArray(this.v[(isAll === true) ? 'childNodes' : 'children']), nb)
                            }
                            res = res.concat(r)
                        });
                        return $GET('NODE').create(res)
                    },
                    nodeelement: function (el, nb, isAll) { return children.node([el], nb, isAll, ZKID) },
                    nodelist: function (el, nb, isAll) { return children.node(toArray(el), nb, isAll, ZKID) },
                    htmlcollection: function (el, nb, isAll) { return children.nodelist(el, nb, isAll) }
                };
                ...
                var html = {
                    node: function (el) {
                        return text.node(el, ZKID)
                    },
                    nodeelement: function (el) {
                        return text.nodeelement(el, ZKID)
                    },
                    nodelist: function (el) {
                        return text.nodelist(el, ZKID)
                    },
                    htmlcollection: function (el) {
                        return html.nodelist(el)
                    }
                };
                // Renvoie le service NODE
                var checked = {
            ...
            function REMOVE() {
                ...
                function removeNode(el, reg, other, opt) {
                    ...
                    if (t !== 'node') {
                        el = $GET('NODE').create((t === 'nodeelement') ? [el] : toArray(el))
                    }
                    return el
                }
                /**
            ...
            function ADD() {
                ...
                // Valeurs possibles
                var addByValueType = {
                    string: function (val, isCreate) { return $GET('NODE').create(val, isCreate).get() },
                    array: function (val, isCreate) {
                        var res = [];
                        each(val, function () {
                            var t = is(this.v);
                            if (addByValueType.hasOwnProperty(t)) {
                                res = res.concat(addByValueType[t](this.v, isCreate))
                            }
                        });
                        return res
                    },
                ...
                var middle = {
                    ...
                    node: function (el, val, zkId) {
                        ...
                        each(nodes, function () {
                            var middleNode = G.middle(toArray(this.v.children))[0];
                            each(newNodes, function () {
                                if (middleNode && this.v) {
                ...
                first = {
                    ...
                    node: function (el, val, last, zkId) {
                        ...
                        each(newNodes, function () { r += (nodeToString(this.v.cloneNode(true))) });
                        each(nodes, function () {
                            if (last) {
                                var node = this.v;
                                each(newNodes, function(){
                                    node.appendChild(this.v);
                                });
                            } else {
                                t = r + this.v.innerHTML;
                                this.v.innerHTML = t
                            }
                        });
                        return el
                ...
                before = {
                    ...
                    node: function (el, reg, val, after, zkId) {
                        ...
                        each(nodes, function () {
                            ...
                            if (beforeNode) {
                                each(newNodes, function () {
                                    after ? insertAfter(this.v, beforeNode) : insertBefore(this.v, beforeNode);
                                    beforeNode = this.v
                                });
                            }
                        });
                ...
                var Attr = {
                        ...
                        nodeelement: function (el, attr, attrVal) { return Attr.node([el], attr, attrVal, ZKID) },
                        nodelist: function (el, attr, attrVal) { return Attr.node(toArray(el), attr, attrVal, ZKID) },
                        htmlcollection: function (el, attr, attrVal) { return Attr.nodelist(el, attr, attrVal) },
                    },
                    attrs = ['Class', 'Id'] ;
                each(attrs, function () {
                ...
                var Style = {
                    ...
                    node: function (el, style, styleVal, zkId) {
                        var nodes = (zkId === ZKID) ? el : el.get() ;
                        el = (zkId === ZKID) ? $GET('NODE').create(nodes) : el ;
                        if(styleVal === undefined){ return el}
                        var newStyle = style + ':' + styleVal ;
                        el = $GET('REMOVE').style(el, style);

                        //var regStyle = RegExp(style+':[^;:]','ig') ;

                        each(nodes, function(){
                            var oldStyle = this.v.getAttribute('style') ;
                 ...
                 * @returns {*}
                 */
                This.x = function (el, val) {
                    val = (val+'')/*.replace(/[^\d\.-]+/,'')+'px'*/;
                    return This.style(el, 'left', val)
                };
                AF.addX = function (val) { return This.x(this, val) } ;
                // Même fonctionnement que This.x
                This.y = function (el, val) {
                    val = (val+'')/*.replace(/[^\d\.-]+/,'')+'px'*/;
                    return This.style(el, 'top', val)
                };
                ...
                var text = {
                    // Renvoie le service NODE
                    node: function (el, text, isConcat, zkId) {
                        var nodes = (zkId === ZKID) ? el : el.get();
                        el = $GET('NODE').create(nodes) ;
                        if(text === undefined){ return el}
                        each(nodes, function () {
                            if(isConcat === true){ this.v.appendChild(document.createTextNode(text)) } else { this.v.textContent = text }
                        });
                        return el
                    },
                    nodeelement: function (el, text, isConcat) { return text.node([el], text, isConcat, ZKID) },
                    nodelist: function (el, text, isConcat) { return text.node(toArray(el), text, isConcat, ZKID) },
                    htmlcollection: function (el, text, isConcat) { return text.nodelist(el, text, isConcat) },
                } ;
                // Si isConcat === true     => Alors on fait une concaténation
                var html = {
                    // Renvoie le service NODE
                    node: function (el, html, isConcat, zkId) {
                        var nodes = (zkId === ZKID) ? el : el.get();
                        el = $GET('NODE').create(nodes) ;
                        if(html === undefined){ return el}
                        each(nodes, function () {
                            if (isConcat === true) {
                                var oldHtml = (this.v.innerHTML) + '' + html ;
                    ...
                    nodeelement: function (el, html, isConcat) { return html.node([el], html, isConcat, ZKID) },
                    nodelist: function (el, html, isConcat) { return html.node(toArray(el), html, isConcat, ZKID) },
                    htmlcollection: function (el, html, isConcat) { return html.nodelist(el, html, isConcat) },
                } ;
                ...
                var addByEmptyOpt = {
                    node: function (el, zkId) { return (zkId === ZKID) ? $GET('NODE').create(el) : el },
                    // Renvoie le service NODE
                    nodeelement: function (el) { return addByEmptyOpt.node([el], ZKID) },
                    // Renvoie le service NODE
                    nodelist: function (el) { return addByEmptyOpt.node(toArray(el), ZKID) },
                    // Renvoie le service NODE
                    htmlcollection: function (el) { return addByEmptyOpt.nodelist(el) }
                } ;
                This.add = function (el, html, isConcat) {
                    if(is(html, /^string|number$/i)){ return This.html(el, html, isConcat) }
                    var t = is(el);
                    return (addByEmptyOpt.hasOwnProperty(t)) ? addByEmptyOpt[t](el) : el
                } ;
        ...
        // ---------------------------------------------------------------
            ...
            // Service MAP
            function MAP() {
                var This = this, nodes = [], G = $GET('GET'), A = $GET('ADD'), N = $GET('NODE'), view = null;

                var Latitude = null, Longitude = null ;

                var coords = {
                    91700: [48.6404067,2.332279900000003],
                    Paris: [48.8785779,2.3641929000000346],
                ...
                function loadScript(){
                    ...
                    script.src = "http://maps.google.com/maps/api/js?callback=loadMap";
                    A.before(document.body, 'script', script);
                }
                ...
                $W.loadMap = function() {
                    ...
                    var map=new google.maps.Map(view.get()[0],mapProp);
                }

                This.view = function(el){
                    if(el === undefined){ return view }
                    if(el === null){ view = null; return This }
                    view = N.create(el).get()[0];
                    if(view){ view = N.create(view) } else { view = null }

                    return This
                };

                This.show = function(postale){
                    if(view){
                        Latitude = coords[postale][0] ; Longitude = coords[postale][1]
                        loadScript();
                    }
        ...
        // ---------------------------------------------------------------
            ...
            function DATE() {
                ...
                function getSetDate(nb, what){
                    ...
                    if (is(nb, 'number')) { nb = Math.abs(nb); if (nb > 0) { try { date['set'+what](nb) } catch (e) { date.setDate(nb) } } return This }
                    if(/[\+\-]\d+/.test(nb)){
                ...
                This.M = function (nb, zkId) {
                    var mois = C[(zkId===ZKID)?'days':'months']()[date['get'+((zkId===ZKID)?'Day':'Month')]()] ;
                ...
                This.DD = function (nb) {
                    if(nb === undefined){ return C.days()[date.getDay()] }
                    return This.D(nb)
                };
                ...
                This.next = function(str){
                    ...
                    var days = C.days();
                    days = days.concat(days).slice(date.getDay()+1) ;
                    var i, k = days.length ;
                    for(i=0 ; i<k ; i++){
                        if(reg.test(days[i])){ date.setDate(date.getDate()+i+1); return This }
                    }
                ...
                This.previous = function(str){
                    ...
                    // Recherche pour les jours
                    var days = C.days();
                    days = days.concat(days.slice(0,date.getDay())) ;
            ...
            $W.$ = function (el, isCreate) { return $GET('NODE').create(el, isCreate) };
            var getThisByValueType = {
                string: function (el, cssSel) {
                    var res = [] ;
                    each(el, function(){
                        var node = this.v, children = node.parentNode.querySelectorAll(cssSel) ;
                        if(children){
                            var i, k = children.length ;
                            for(i = 0 ; i < k ; i++){
                                if(children[i] === node){ res.push(node); i = k+1 }
                            }
                        }
                    }) ;
                    return res
                },
                array: function (el, tab) { return $GET('GET').at(el, tab) },
                number: function(el, num){ return $GET('GET')[(num<0)?'last':'first'](el, num) },
            } ;
            ...
            function NODE() {
                ...
                var submenuID = 'submenu'+parseInt(Math.random() * 100000), configForSubmenu = {
                    ...
                    view: 'ul.zk-submenu',
                    viewEvent: 'out',
                    viewFunc: function(){ var view = $GET('NODE').create(this.node); removeItemsEventFunc(view); view.remove() } ,
                    itemsView: 'li.zk-submenu-items',
                    itemsEvent: 'click',
                    itemsFunc: function(){ This.create('.zk-submenu').remove() },
                    position: 'bottom',
                    fullscreen: false,
                } ;
                ...
                var createByObj = {
                    ...
                    array: function (el) {
                        ...
                        each(el, function () {
                            var v = this.v, t = is(v);
                            if (createByObj.hasOwnProperty(t)) {
                                n = n.concat(createByObj[t](v))
                            }
                        });
                        return n
                    },
                ...
                This.create = function (el, isCreate) {
                    if (isCreate === true) {
                        if (is(el, 'string')) { nodes = createElementByCss(el) }
                    } else {
                        var t = is(el);
                        if (createByObj.hasOwnProperty(t)) { nodes = createByObj[t](el) }
                    }
                    return This
                } ;
                This.get = function (reg) {
                    if (reg === undefined) { return nodes }
                    return G.get(This, reg)
                } ;
                /**
                 * Permet de sélectionner des éléments dans la tableau nodes
                 *      - val = Array                       => Alors  G.at(el, val)
                 *      - val = Number positif              => Alors  G.first(el, val)
                 *      - val = Number négatif              => Alors  G.last(el, val)
                 *      - val = String                      => Alors  on renvoie un service NODE contenant les objets correspondant au css selector
                 */
                This.this = function(val){
                    var t = is(val) ;
                    if(getThisByValueType.hasOwnProperty(t)){ nodes = getThisByValueType[t](nodes, val) ; return This }
                    nodes = [] ; return This
                } ,
                // FAIRE LA METHODE NEXT PREVIOUS
                This.remove = function (reg) { return $GET('REMOVE').remove(This, reg) } ;
                This.add = function (html, isConcat) { return A.add(This, html, isConcat) } ;
                ...
                This.style = function (style, val) { return (style === undefined) ? G.style(This) : A.style(This, style, val) };
                var styles = ['color','margin', 'padding', 'border', 'width', 'width1', 'width2', 'height', 'height1', 'height2', 'x', 'innerX', 'y', 'innerY','background','cursor','position'] ;
                each(styles, function () {
                    var v = this.v;
                    This[v] = function (val) { return (val === undefined) ? G.style(This, v) : This.style(v, val) }
                });
                ...
                // L'adapter pour le sélecteur de style
                This.children = function (nb, isAll) { return G.children(This, nb, isAll) };
                ...
                //This.innerX = function () { return G.innerX(This) };
                This.y = function (val) { return (val === undefined) ? G.y(This) : A.y(This, val) };
                //This.innerY = function () { return G.innerY(This) };
            ...
            function EVENT() {
                ...
                var onByObj = {
                    // Renvoie un tableau
                    node: function (el, ev, func, args, zkId) {
